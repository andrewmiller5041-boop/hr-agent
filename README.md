...
3. Copy `.env.example` to `.env` and set `GROQ_API_KEY` (free at
   ...
   run locally via Chroma's built-in ONNX MiniLM model (`onnxruntime`, no
   torch required).
...
4. Deploy. First boot builds the Chroma index from `corpus/` (a few seconds
   given the corpus size) and downloads the MiniLM embedding model from
